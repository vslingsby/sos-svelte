...
<Row>
  <Col><input type="text" bind:value={searchString} /></Col>
  <Col>
    <Button
      on:click={() => {
        addNewBoon = false;
...
{#each boonList.filter((boon) => boon.name
      .toLowerCase()
      .indexOf(searchString) != -1) as boon}
  <Row>
    <div class="col-sm-2">
      <p>
        <strong>{boon.name}</strong>
        ({#each boon.cost as level, i}
          {level}{#if i < boon.cost.length - 1}/{/if}
        {/each})
      </p>
    </div>
    <Col>
